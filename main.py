...
setup_logging(
    os.path.join(os.path.dirname(__file__), "Utils/logging.yaml"), Logs_path="Logs"
)
...
if __name__ == "__main__":
    ...
    result01 = db_instance.mean_bought_given_user(
        user=cfg["queries"]["mean_bought_given_user"]["parameters"]["user"]
    )

    result02 = db_instance.sales_mean_by_month_given_year(
        year=cfg["queries"]["sales_mean_by_month_given_year"]["parameters"]["year"]
    )
